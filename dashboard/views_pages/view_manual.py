...
def get_response(request):
    ...
    if request.method == "POST":
        if 'req' in request.POST:
            ...
            elif 'uniswap_fiat_to_token_amount' in request.POST:
                fiat_to_token_amount = eval(request.POST['uniswap_fiat_to_token_amount'])
            ...
            elif 'uniswap_token_to_fiat_amount' in request.POST:
                token_to_fiat_amount = eval(request.POST['uniswap_token_to_fiat_amount'])
                coin = request.POST['coin']
                transaction = transaction_dispatch.create_and_actualize_uniswap_token_to_fiat_transaction(token_to_fiat_amount, coin=coin)


            elif 'uniswap_eth_amount_to_wrap' in request.POST:
                eth_amount_to_wrap = eval(request.POST['uniswap_eth_amount_to_wrap'])
                transaction = transaction_dispatch.create_and_actualize_uniswap_wrap_eth(eth_amount_to_wrap)

            elif 'uniswap_weth_amount_to_unwrap' in request.POST:
            ...
            elif 'sushiswap_fiat_to_token_amount' in request.POST:
                fiat_to_token_amount = eval(request.POST['sushiswap_fiat_to_token_amount'])
                transaction = transaction_dispatch.create_and_actualize_sushiswap_fiat_to_token_transaction(fiat_to_token_amount, coin='weth')
